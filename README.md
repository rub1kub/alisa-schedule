# ККЭПик — расписание ККЭП для Алисы

Навык сообщает расписание ККЭП на сегодня, завтра или выбранный день. Источник — [ККЭПик](https://kkepik.rub1kub.ru/api/docs/), программа, которая получает расписание колледжа.
...
После полного ответа навык сам закрывается. Он продолжает слушать, если нужно уточнить группу, день или дочитать длинное расписание.

Запуск: **«Алиса, спроси у ККЭПика, что завтра»**. О настройке своей фразы — в [LAUNCH.md](docs/LAUNCH.md).
...
«Какие предметы завтра» и «кто ведёт пары» задают формат одного ответа. Команды «называй предметы», «называй преподавателей», «предметы и преподаватели» меняют сохранённый выбор. «Выбирай по курсу» возвращает автоматический режим.
...
flowchart LR
    ...
    C --> D[Диалог: группа, дата, вопрос]
    D --> E[Шаблоны ответов JSON]
    D --> F[ScheduleProvider]
    F --> G[Локальные замены JSON]
    F --> H[Адаптер и кэш]
    H -->|GET: группа и точная дата| I[API ККЭПика]
    D -->|Группа и предпочтение| A
```

Backend работает отдельным сервисом. Он читает два публичных маршрута API ККЭПика. Код, БД и файловая структура бота и веб-приложения не меняются.

API уже возвращает расписание на конкретную дату, поэтому расчёт недель и PDF-парсинг остаются у источника. Адаптер переводит строки API в занятия; диалог работает с этими объектами. Другой источник подключается через `groups()` и `day(group_id, date)`.
...
- «Смени группу», «список групп», «забудь группу», «моя группа», «помощь», «выход»: управление диалогом.
