...
def detect_intent(command: str, nlu: NLU) -> str | None:
    ...
    for intent, commands in preferences.items():
        if text in commands:
            return intent
    if re.search(
        r"\b(?:забудь|сбрось|удали)\b.*\b(?:групп\w*|расписание|профиль|фамилию|преподавателя)",
        text,
    ):
        return "forget"
    if text in {
        ...
        "мой профиль",
        "мои настройки",
        "какое расписание сохранено",
        "мой преподаватель",
        "моя фамилия",
    }:
        return "my_profile"
    if re.search(
        r"\b(?:смени|сменить|поменяй|выбери|выбрать|измени|изменить|запомни)\b"
        r".*\b(?:расписание|профиль)\b",
        text,
    ):
        return "change_profile"
    if re.match(
        r"^(?:я (?:преподаватель|препод)|режим преподавателя|моя фамилия|мой преподаватель|"
        r"(?:смени|поменяй|выбери|запомни) (?:мою |моего )?(?:фамилию|преподавателя))\b",
        text,
    ):
        return "choose_teacher"
    if text in {"я студент", "я студентка"} or re.match(
        r"^(?:моя группа|учусь в группе)\s+",
        text,
    ):
        return "change"
    if re.search(
